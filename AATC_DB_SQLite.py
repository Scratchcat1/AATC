...
########################    DRONE           ################################ 
    ...
    def GetDroneInfo(self,UserID,DroneID):
        self.cur.execute("SELECT DISCRETE Drone.* FROM Drone,User WHERE Drone.DroneID = ? AND (Drone.UserID = ? OR (Drone.UserID = User.UserID AND User.PublicVisibleFlights = 1))",(DroneID,UserID))
    ...
    def UpdateDroneStatus(self,DroneID,LastCoords,LastBattery):
        self.cur.execute("UPDATE Drone SET LastCoords = ?,LastBattery = ? WHERE DroneID = ?",(str(LastCoords),LastBattery,DroneID))
        self.db_con.commit()
...
#####################          FLIGHT          ##############################
    ...
    def CheckForFlight(self,DroneID,MaxLookAheadTime):
        self.cur.execute("SELECT FlightID FROM Flight WHERE DroneID = ? AND StartTime < (?-?) ORDER BY StartTime ASC LIMIT 1",(DroneID,GetTime(),MaxLookAheadTime))
    ...
    def MarkFlightComplete(self,DroneID,FlightID,Code):
        ...
        if self.cur.fetchall != []:
            self.cur.execute("UPDATE Flight SET Completed = ?,EndTime = ? WHERE FlightID = ?",(Code,FlightID,GetTime()))
            self.db_con.commit()
