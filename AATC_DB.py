...
########################    DRONE           ################################ 
    ...
    def DroneCheckCredentials(self,DroneID,DronePassword):
        self._cur.execute("SELECT DroneID FROM DroneCredentials WHERE DroneID = %s AND DronePassword = %s",(DroneID,DronePassword))
    ...
    def SetDroneCredentials(self,UserID,DroneID,DronePassword):
        self._cur.execute("SELECT 1 FROM Drone WHERE Drone.UserID = %s AND Drone.DroneID = %s",(UserID,DroneID))
        if self._cur.fetchall() != ():
...
##########################   USER         #############################
    ...
    def AddUser(self,Username,Password):
        self._cur.execute("SELECT 1 FROM User WHERE Username = %s",(Username,))
        if self._cur.fetchall() == ():
    ...
    def CheckCredentials(self,Username,Password):
        self._cur.execute("SELECT UserID FROM User WHERE Username = %s AND Password = %s",(Username,Hash(Password,Username)))
    ...
    def UserChangePassword(self,UserID,OldPassword,NewPassword):
        self._cur.execute("SELECT Username from User WHERE UserID = ?",(UserID,))
        Username = self._cur.fetchall()[0][0]
        self._cur.execute("SELECT 1 FROM User WHERE UserID = %s and Password = %s",(UserID,Hash(OldPassword,Username)))
        if self._cur.fetchall() != ():
            self._cur.execute("UPDATE User SET Password = %s WHERE UserID = %s",(Hash(NewPassword,Username),UserID))
            self._db_con.commit()
...
#####################          FLIGHT          ##############################
    ...
    def AddFlight(self,UserID,DroneID,StartCoords,EndCoords,StartTime,ETA,EndTime,Distance,XOffset,YOffset,ZOffset):
        self._cur.execute("SELECT 1 FROM User,Drone WHERE Drone.DroneID = %s AND Drone.UserID = %s",(DroneID,UserID))
        if self._cur.fetchall() !=():
    ...
    def RemoveFlight(self,UserID,FlightID):
        self._cur.execute("SELECT 1 FROM Flight,Drone,User WHERE Flight.FlightID = %s AND Flight.DroneID = Drone.DroneID AND Drone.UserID = %s",(FlightID,UserID))  #If User owns this drone
        if self._cur.fetchall() != ():
    ...
    def CheckForFlight(self,DroneID,MaxLookAheadTime):
        ...
        self._cur.execute("SELECT FlightID FROM Flight WHERE DroneID = %s AND StartTime < (%s+%s) AND StartTime > (%s-%s) AND Completed = 0 ORDER BY StartTime ASC LIMIT 1",(DroneID,GetTime(),MaxLookAheadTime, GetTime(),InvalidateDelay))
    ...
    def MarkFlightComplete(self,DroneID,FlightID,Code):
        self._cur.execute("SELECT 1 FROM Flight WHERE DroneID = %s AND FlightID = %s",(DroneID,FlightID))
        if self._cur.fetchall() != ():
...
#----------------------------  FLIGHT WAYPOINT------------------------------------
    ...
    def AddWaypoint(self,UserID,FlightID,WaypointNumber,Coords,ETA,BlockTime=0):
        self._cur.execute("SELECT 1 FROM User,Flight,Drone WHERE User.UserID = %s AND User.UserID = Drone.UserID AND Drone.DroneID = Flight.DroneID AND Flight.FlightID = %s",(UserID,FlightID))
        if self._cur.fetchall() !=():
...
###########################  MONITOR     ##################################
    def AddMonitor(self,MonitorName,MonitorPassword):
        self._cur.execute("SELECT 1 FROM Monitor WHERE MonitorName = %s",(MonitorName,))
        if self._cur.fetchall() == ():
            self._cur.execute("INSERT INTO Monitor(MonitorName,MonitorPassword,AccountType) VALUES(%s,%s,'Default')",(MonitorName,Hash(MonitorPassword,MonitorName)))
            self._db_con.commit()
    ...
    def MonitorCheckCredentials(self,MonitorName,MonitorPassword):
        self._cur.execute("SELECT MonitorID FROM Monitor WHERE MonitorName = %s AND MonitorPassword = %s",(MonitorName,Hash(MonitorPassword,MonitorName)))
    ...
    def MonitorChangePassword(self,MonitorID,OldPassword,NewPassword):
        self._cur.execute("SELECT MonitorName FROM Monitor WHERE MonitorID = ?",(MonitorID,))
        MonitorName = self._cur.fetchall()[0][0]
        self._cur.execute("SELECT 1 FROM Monitor WHERE MonitorID = %s AND MonitorPassword = %s",(MonitorID,Hash(OldPassword,MonitorName)))
        if self._cur.fetchall() != ():
...
#--------------------------   MONITOR PERMISSION   ------------------------------------
    def AddMonitorPermission(self,UserID,MonitorID,ExpiryDate):
        self._cur.execute("SELECT 1 FROM MonitorPermission WHERE UserID = %s AND MonitorID = %s",(UserID,MonitorID))
        if self._cur.fetchall() ==():
...
########################    No Fly Zone ###############################################
    def AddNoFlyZone(self,Coord1,Coord2,Level,UserID):
        if not CoordLessThanOrEqual(Coord1,Coord2):  #If Coord2 is closer to origin in every point than Coord1 It is an invalid coordinate
            ...
            self._cur.execute("SELECT 1 FROM User WHERE UserID = %s AND ZoneCreatorPermission =1",(UserID,))
            if self._cur.fetchall() !=  ():
...
####self._cur.execute("CREATE TABLE User(UserID INTEGER PRIMARY KEY AUTO_INCREMENT, Username TEXT,Password TEXT, PublicVisibleFlights INT, AccountType TEXT)")
...
####self._cur.execute("CREATE TABLE Monitor(MonitorID INTEGER PRIMARY KEY AUTO_INCREMENT, MonitorName TEXT, MonitorPassword TEXT,AccountType TEXT)")
...
####self._cur.execute("CREATE TABLE FlightWaypoints(FlightID INT, WaypointNumber INT, Coords TEXT, ETA REAL, BlockTime INT ,FOREIGN KEY(FlightID) REFERENCES Flight(FlightID))")
...
####self._cur.execute("CREATE TABLE DroneCredentials(DroneID INTEGER PRIMARY KEY AUTO_INCREMENT ,DronePassword TEXT,FOREIGN KEY(DroneID) REFERENCES Drone(DroneID))")
